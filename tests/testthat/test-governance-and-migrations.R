...
testthat::test_that("revision logging and as-of extraction work on a second vintage", {
  ...
  # The audit's F-04, on the same two vintages: blocking withdraws the bundle
  # from every *current* research interface at once, and the unfiltered twin
  # still holds everything so the release can be diagnosed. Nothing is rebuilt to
  # do this -- publication is the one-row pointer, so clearing it is the whole
  # decision.
  publish_test_release(con, "release:test", "blocked", errors = 1L)
  testthat::expect_equal(DBI::dbGetQuery(
    con, "SELECT count(*) AS n FROM v_series_latest"
  )$n[[1]], 0L)
  # As-of does *not* go to zero, and that changed in schema 36 for a reason.
  #
  # This bundle was accepted and published before it was blocked, so on
  # 2026-06-01 a researcher genuinely could have read it. A later failed rebuild
  # does not un-happen an earlier publication, and an as-of interface that
  # erased history whenever the newest attempt failed would be rewriting the
  # past to match the present -- the same class of error, in the opposite
  # direction, as the look-ahead RA2-01 found.
  #
  # A bundle that was *never* accepted is a different case and stays invisible
  # at every cutoff; test-asof-history.R asserts that half.
  testthat::expect_equal(DBI::dbGetQuery(
...
testthat::test_that("detailed trade carries explicit economic dimensions, not a parsed row label", {
  ...
  # different sheets and is asserted in test-grain-and-provenance.R; an unscoped
