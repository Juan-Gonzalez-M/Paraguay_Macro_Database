...
paths <- list(
  ...
  candidate = file.path(root, "database", "candidates",
                        "accepted_for_review_20260914_202049.duckdb")
)
...
expected_sha <- c(
  ...
  candidate = "6079c80ce0edd26f899b9f54fe98f73eee08c90c5f006759b051960edbd7a9fa"
)
...
candidate_release <- DBI::dbGetQuery(attached, paste(
  ...
  "WHERE d.build_id='build:a3e94ab54dacd47000223d28'"
))
...
candidate_artifact <- DBI::dbGetQuery(attached, paste(
  "SELECT artifact_path,size_bytes,sha256,artifact_role",
  "FROM candidate.audit.distribution_artifacts",
  "WHERE build_id='build:a3e94ab54dacd47000223d28'"
))
stopifnot(
  nrow(candidate_artifact) == 1L,
  candidate_artifact$artifact_path ==
    "candidates/accepted_for_review_20260914_202049.duckdb",
  candidate_artifact$size_bytes == file.info(paths$candidate)$size,
  is.na(candidate_artifact$sha256),
  candidate_artifact$artifact_role == "database",
  !grepl("paraguay_macro_pilot", candidate_artifact$artifact_path, fixed = TRUE)
)
emit("candidate_recorded_artifact_path", candidate_artifact$artifact_path)
emit("candidate_recorded_artifact_bytes", candidate_artifact$size_bytes)
...
deferred <- DBI::dbGetQuery(attached, paste(
  ...
  "WHERE attempt_id='attempt:2305af6de17f489900d0afb8'",
...
release_errors <- scalar(attached, paste(
  ...
  "WHERE attempt_id='attempt:2305af6de17f489900d0afb8' AND severity='error'"
))
...
# The independently accepted lineage artifact is presently retained under
# /private/tmp. Repeat the same complete comparisons against the repository-held
# frozen incumbent so the lineage-only population result remains reproducible
# after that temporary review artifact is retired.
fact_candidate_only_frozen <- scalar(attached, paste(
  "SELECT count(*) FROM (SELECT * FROM candidate.canonical.fact_series_events",
  "EXCEPT ALL SELECT * FROM frozen.canonical.fact_series_events)"
))
fact_frozen_only <- scalar(attached, paste(
  "SELECT count(*) FROM (SELECT * FROM frozen.canonical.fact_series_events",
  "EXCEPT ALL SELECT * FROM candidate.canonical.fact_series_events)"
))
series_candidate_only_frozen <- scalar(attached, paste(
  "SELECT count(*) FROM (SELECT * FROM candidate.canonical.dim_series",
  "EXCEPT ALL SELECT * FROM frozen.canonical.dim_series)"
))
series_frozen_only <- scalar(attached, paste(
  "SELECT count(*) FROM (SELECT * FROM frozen.canonical.dim_series",
  "EXCEPT ALL SELECT * FROM candidate.canonical.dim_series)"
))
emit("candidate_only_canonical_facts_vs_frozen", fact_candidate_only_frozen)
emit("frozen_only_canonical_facts_vs_candidate", fact_frozen_only)
emit("candidate_only_canonical_series_vs_frozen", series_candidate_only_frozen)
emit("frozen_only_canonical_series_vs_candidate", series_frozen_only)
stopifnot(all(c(
  fact_candidate_only_frozen, fact_frozen_only,
  series_candidate_only_frozen, series_frozen_only
) == 0L))
