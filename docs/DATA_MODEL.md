...
`available_at` is when the vintage became available to this project, and it is never inferred from the reference period. Since schema 28 it is taken in order of authority: the operator-recorded availability in `raw.source_provenance` where one exists, then the vintage's publication date, then the timestamp of first ingestion. The first is the only one that is a fact about the publisher rather than about this machine.
